...
from .todolist import clova_api
...
def index(request):
    result_list = clova_api()
    generated_summary = summary()  # Call the summary function to generate the summary content
    return render(request, 'blog/blog.html', {'result_list': result_list, 'generated_summary': generated_summary})

@csrf_exempt
def chat(request):
    if request.method == 'POST':
        # user_message = request.POST.get('question')
        # result = chatGPT(user_message)  # ChatGPT와 대화를 주고받는 함수
        # response_data = {
        #     'result': result
        # }
...
def page_view(request):
    ...
    return render(request, 'blog/Page.html', {'generated_summary': generated_summary}) 